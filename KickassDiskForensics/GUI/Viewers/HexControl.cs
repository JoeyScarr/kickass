...
namespace KFA.GUI.Viewers {
	public partial class HexControl : UserControl, IDataViewer {
		...
		public Byte[] GetSelectedBytes() {
			m_dataStream.Open();
			byte[] result = m_dataStream.GetBytes(m_selectionStart, m_selectionEnd - m_selectionStart);
			m_dataStream.Close();
			return result;
		}
		...
		private void Update(ulong row) {
			if (m_dataStream != null) {
				...
				for (ulong rowOffset = startOffset; rowOffset < endOffset; rowOffset += 16) {
					byte[] line = m_dataStream.GetBytes(rowOffset, Math.Min((ulong)16, endOffset - rowOffset));
					hexString.Append(ConvertToHexString(line));
					hexString.Append('\n');
					asciiString.Append(ConvertToString(line));
					asciiString.Append('\n');
				...
				txtBoxHex.Text = hexString.ToString();
				txtBoxAscii.Text = asciiString.ToString();
			}
		}
		...
		private void ParseData(ulong offset) {
			if (m_dataStream != null) {
				...
				if ((long)offset <= (long)m_dataStream.StreamLength - 1) {
					tbByte.Text = Util.GetByte(m_dataStream, offset).ToString();
				} else {
					tbByte.Text = "";
				}
